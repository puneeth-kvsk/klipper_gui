...
def move_axis_to(axis, value):
    ...
    if axis == 'J':
        command_pool.extend([low_level_functions.gcodeUrl + absolute_position_command, low_level_functions.gcodeUrl + move_axis_command + axis + str(value + 30)])
    else:
        command_pool.extend([low_level_functions.gcodeUrl + absolute_position_command, low_level_functions.gcodeUrl + move_axis_command + axis + str(value)])
    return command_pool
...
def move_axis_forward_by(axis, value):
    command_pool = []
    command_pool.extend(low_level_functions.set_max_vel_accel_auto(str(axis)))
    command_pool.extend([low_level_functions.gcodeUrl + relative_position_command, low_level_functions.gcodeUrl + move_axis_command + axis + str(value)])
    return command_pool
...
def move_multiple_axes_to(params):
    ...
    command_pool.append(low_level_functions.gcodeUrl + move_axis_command)
